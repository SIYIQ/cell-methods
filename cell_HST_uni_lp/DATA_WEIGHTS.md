# cell_HST_uni_lp 所需数据与权重

本目录包含冻结 UNI + PCA + Ridge 线性探针源码。**模型权重、缓存与训练输出未包含。**
...
- **UNI** 权重：`uni_local_path`（默认 `/home/sb202604/UNI`）
  - 需要文件：`pytorch_model.bin`、`config.json` 等。
...
- `cache/<dataset>/` — 编码后的 UNI CLS 特征 (`*_uni_cls.npy`、`*_he.npy`) 及 manifest 文件。
