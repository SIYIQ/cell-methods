...
将 `HST_uni_lp`（冻结 UNI + PCA + Ridge 线性探针）适配到单细胞 Xenium 数据上的对比方法。
...
- `config.yaml` / `config_processed_cell.yaml` 中的 `processed_root`、`processed_cell_root`、`uni_local_path`。
...
```bash
cd cell_level_methods/cell_HST_uni_lp
...
编码后的 UNI 特征缓存到 `./cache/<dataset>/<half>_uni_cls.npy`，并附带 `.manifest.json` 校验源 `patches.npy`，避免重复编码。
