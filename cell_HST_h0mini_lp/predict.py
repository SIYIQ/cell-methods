"""H0-mini single-cell Xenium linear-probe evaluation.
...
def run_one_fold(
    ...
    if use_pc:
        ...
        img_batch_size = int(cfg.get("img_batch_size", 64))
        train_cache = cache_root / ds_name / f"{train_half['label']}_h0mini_cls.npy"
        test_cache = cache_root / ds_name / f"{test_half['label']}_h0mini_cls.npy"
...
def main():
    ...
    if not use_pc:
        # Load encoder once
        local_path = cfg["h0mini_local_path"]
        print(f"Loading H0-mini from: {local_path}")
        model, preprocess, feature_dim = load_h0mini(local_path, device)
