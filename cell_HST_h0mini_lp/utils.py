"""Shared utilities for cell_HST_h0mini_lp.

Adapts the frozen H0-mini + PCA + Ridge linear-probe protocol from
HST_h0mini_lp to the single-cell Xenium benchmark in
...
# =============================================================================
# H0-mini loading + encoding
# =============================================================================
def load_h0mini(local_path, device):
    """Load H0-mini ViT from a local HuggingFace snapshot directory.

    Returns ``(model, preprocess, embedding_dim)`` where ``preprocess`` maps
    a PIL.Image -> normalized tensor with H0-mini's pretrained statistics.
    """
    import timm
    ...
    mean = pcfg.get("mean", (0.5, 0.5, 0.5))
    std = pcfg.get("std", (0.5, 0.5, 0.5))
    ...
    embedding_dim = int(model_cfg.get("num_features", 768))
    print(
        f"Loaded H0-mini from {local_path}: dim={embedding_dim}, "
