"""cell_HST_middle test script: compute per-direction and aggregated MSE/MAE.
...
If ``result.json`` contains ``gene_names``, the ground truth is subset to those
genes before computing metrics (matches the 50-HVG training protocol of
cell_HST_middle).
...
def subset_genes(adata, gene_names):
    """Return dense expression matrix [N, len(gene_names)] from adata."""
    common = [g for g in gene_names if g in adata.var_names]
    if len(common) != len(gene_names):
        missing = set(gene_names) - set(common)
        raise ValueError(f"Missing genes in adata: {sorted(missing)}")
...
def evaluate_dataset(dataset: str, output_root: Path, seeds: list[int]):
    ...
    for seed in seeds:
        ...
        if gene_names is not None:
            y1 = subset_genes(a1, gene_names)
            y2 = subset_genes(a2, gene_names)
        else:
            y1 = to_dense(a1.X)
            y2 = to_dense(a2.X)
    ...
    summary = {
        ...
        "n_genes": int(all_dir_results[0]["n_genes"]),
        "seeds": seeds,
...
def main():
    ap = argparse.ArgumentParser(
        description="Aggregate cell_HST_middle predictions and compute MSE/MAE."
    )
